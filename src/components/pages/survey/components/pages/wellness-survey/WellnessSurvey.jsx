...
const WellnessSurvey = (props) => {
  ...
  const error = (event) => {
    ...
    const message = {
      notifStatus: true,
      notifMessage: "Answer all the Wellness Questions first before continue on the next Survey"
    }
  ...
  return (
    <div className='wellnessSurvey'>
      <div className='wellnessSurveyContent'>
        <form onSubmit={questionsState.length === questionActivate ? finishSurvey : error}>
          {
            questionsState?.map((obj, idx) => {
              return(      
                <div>
